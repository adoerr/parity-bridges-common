...
use relay_substrate_client::{
	headers_source::HeadersSource, BlockNumberOf, Error as SubstrateError, HashOf, HeaderOf, JustificationsSubscription,
};
						...
						match justification_target {
							Ok((target_hash, target_number)) => {
								...
								if sync.headers().requires_completion_data(&target) {
									sync.headers_mut()
										.completion_response(&target, Some(millau_justification));
									continue;
								}
...
pub async fn run(millau_client: MillauClient, rialto_client: RialtoClient, rialto_sign: RialtoSigningParams) {
